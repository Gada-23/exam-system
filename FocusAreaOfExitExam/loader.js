...
const COURSE_CONFIG = {
  artificialIntelligence: {
    ...
    icon: "fa-robot",
    description:
      "Complete focus notes for AI exit exam preparation covering intelligent agents, search algorithms, knowledge representation, and machine learning.",
    totalQuestions: 6,
  ...
  computerSecurity: {
    ...
    icon: "fa-shield-alt",
    description:
      "Comprehensive security focus notes covering cryptography, network security, access control, and security policies.",
    totalQuestions: 6,
  ...
  operatingSystem: {
    ...
    icon: "fa-windows",
    description:
      "Complete OS focus notes covering process management, memory management, file systems, and scheduling algorithms.",
    totalQuestions: 6,
  ...
  dataCommunicationNetworking: {
    ...
    description:
      "Networking focus notes covering OSI model, TCP/IP, protocols, routing, and network security.",
    totalQuestions: 6,
  ...
  compilerDesign: {
    ...
    icon: "fa-file-code",
    description:
      "Compiler focus notes covering lexical analysis, parsing, semantic analysis, and code generation.",
    totalQuestions: 6,
  ...
  automataComplexityTheory: {
    ...
    icon: "fa-infinity",
    description:
      "Theory focus notes covering finite automata, regular languages, context-free grammars, and Turing machines.",
    totalQuestions: 6,
  ...
  computerOrganizationArchitecture: {
    ...
    icon: "fa-microchip",
    description:
      "Computer architecture focus notes covering CPU design, memory hierarchy, pipelining, and I/O systems.",
    totalQuestions: 6,
  ...
  networkSystemAdministration: {
    ...
    icon: "fa-server",
    description:
      "System administration focus notes covering user management, network services, security policies, and scripting.",
    totalQuestions: 6,
  ...
  fundamentalDatabaseSystems: {
    ...
    icon: "fa-database",
    description:
      "Core database concepts including design principles, normalization, ER modeling, and real-world database applications.",
    totalQuestions: 6,
  ...
  advancedDatabaseSystems: {
    ...
    description:
      "Advanced database topics including object-oriented databases, recovery methods, distributed databases, and query optimization.",
    totalQuestions: 6,
  ...
  databaseSystems: {
    ...
    icon: "fa-database",
    description:
      "Database focus notes covering SQL, normalization, transactions, and database design.",
    totalQuestions: 6,
  ...
  dataStructuresAlgorithms: {
    ...
    icon: "fa-sitemap",
    description:
      "Data structures focus notes covering arrays, linked lists, trees, graphs, and algorithm analysis.",
    totalQuestions: 6,
  ...
  softwareEngineering: {
    ...
    icon: "fa-cog",
    description:
      "Software engineering focus notes covering SDLC, agile methodologies, testing, and design patterns.",
    totalQuestions: 6,
  ...
  webProgramming: {
    ...
    icon: "fa-globe",
    description:
      "Web development focus notes covering HTML, CSS, JavaScript, and modern frameworks.",
    totalQuestions: 6,
  ...
  designAnalysisAlgorithms: {
    ...
    icon: "fa-chart-line",
    description:
      "Algorithm design focus notes covering divide and conquer, dynamic programming, greedy algorithms, and complexity.",
    totalQuestions: 6,
...
async function loadAllChapters() {
  ...
  console.log(
    `   Total: ${loadedCount}/${totalCount} chapters loaded successfully`,
  );
  ...
  Object.entries(courseStats).forEach(([course, stats]) => {
    console.log(
      `   ${course}: ${stats.loaded.length}/${stats.total} chapters - Chapters: ${stats.loaded.join(", ")}`,
    );
  });
  ...
  loadResults.forEach((result) => {
    if (result.loaded) {
      if (window[result.varName]) {
        ...
        console.log(
          `   ❌ ${result.varName} was loaded but variable not set! Check file content`,
        );
        // if script loaded but the expected variable isn't present, look for similarly-prefixed globals
        const prefix = result.varName.replace(/\d+$/, "");
        const alternatives = Object.keys(window).filter(
          (k) => k.startsWith(prefix) && k !== result.varName,
        );
        if (alternatives.length) {
          console.log(
            `       🔍 Found other variables with prefix ${prefix}: ${alternatives.join(", ")}`,
          );
        }
      }
    }
...
function buildCourseStructure(courseStats) {
  ...
  for (const [courseKey, course] of Object.entries(COURSE_CONFIG)) {
    ...
    for (let chapterNum = 1; chapterNum <= course.maxChapters; chapterNum++) {
      ...
      if (chapterData) {
        chapters.push({
          ...
          description:
            chapterData.description || `Chapter ${chapterNum} content`,
          data: chapterData,
        });
    ...
    if (chapters.length > 0) {
      ...
      let expectedChapters =
        EXPECTED_CHAPTER_COUNTS[courseKey] || chapters.length;
      ...
      console.log(
        `✅ Added ${course.title} with ${chapters.length}/${expectedChapters} chapters: ${loadedChapters.join(", ")}`,
      );
    }
  }
  ...
  Object.entries(window.focusArea).forEach(([key, course]) => {
    console.log(
      `   ${course.title}: ${course.chapters.length}/${course.expectedChapters} chapters`,
    );
